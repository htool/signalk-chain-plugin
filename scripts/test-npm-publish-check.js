...
assert.strictEqual(check.isPublishablePath('public/app.js'), true)
...
const publish = check.shouldPublish({
  ...
  changedFiles: ['plugin/index.js', 'public/app.js', 'README.md']
})
...
const missingErr = new Error('NPM_MISSING')
missingErr.code = 'NPM_MISSING'
const missing = check.decide({
  cwd: require('path').join(__dirname, '..'),
  npmView: function () {
    throw missingErr
  }
})
assert.strictEqual(missing.publish, true)
assert.strictEqual(missing.reason, 'package not on registry')
assert.strictEqual(missing.nextVersion, require('../package.json').version)
