#!/usr/bin/env node
/**
 * Decide whether to patch-bump and npm publish signalk-chain-plugin.
 *
 * Publishes at most once per UTC day when plugin/ or public/ changed
 * since the last npm release. First publish (package missing on npm) uses
...
const PACKAGE_NAME = 'signalk-chain-plugin'
const TRACKED_PATHS = ['plugin', 'public']
...
function runNpmView(pkgName, args) {
  ...
  if (r.status !== 0) {
    const err = (r.stderr || r.stdout || '').trim()
    if (/E404|404|not in this registry|is not in this registry/i.test(err)) {
      const missing = new Error('NPM_MISSING')
      missing.code = 'NPM_MISSING'
      throw missing
    }
...
function decide(opts) {
  ...
  try {
    ...
    if (err && err.code === 'NPM_MISSING') {
      return {
        publish: true,
        reason: 'package not on registry',
        files: [],
